...
function App() {
  ...
  const handleInputChange = (e) => {
    ...
    //console.log(e.target.name);//可以知道是哪一項的資料

    const { value, name } = e.target;

    setAccount({
      ...account,
      [name]: value //輸入處可以改變成使用者所輸入的值
    })
  }
  ...
  const handleLogIn = (e) => {
    ...
    .then((res) => {
      ...
      document.cookie = `ShilohToken=${token}; expires=${new Date (expired)}`;

      console.log(`${BASE_URL}/v2/api/${API_PATH}/admin/products`);

      axios.defaults.headers.common['Authorization'] = token;//加上這行之後axios發送請求後都會帶上token
